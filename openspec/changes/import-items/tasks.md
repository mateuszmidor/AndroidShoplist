...
- [ ] 1.1 Create `domain/ListonicImportParser.kt` in package
...
- [ ] 1.2 Implement the parsing rules: split on `\n` (tolerating `\r\n`), trim
...
- [ ] 2.1 Add `@Insert suspend fun insertAll(items: List<ShoppingItemEntity>)`
  to `ShoppingItemDao` (single transaction → all-or-nothing)
- [ ] 2.2 Add `suspend fun createAll(listId: UUID, names: List<String>):
  List<UUID>` to the `ShoppingItemRepository` interface
- [ ] 2.3 Implement `createAll` in `RoomShoppingItemRepository`: generate a UUID
...
- [ ] 3.1 Add `fun importItems(text: String)` to `ItemsViewModel`: parse via
...
- [ ] 4.1 Turn the FAB into an anchored menu: keep the `FloatingActionButton`
...
- [ ] 4.2 Add a private multi-line import dialog inside `ItemsScreen.kt` (per
...
- [ ] 4.3 Add the `onImportText: (String) -> Unit` parameter to `ItemsScreen`
  and wire the FAB menu items to the create dialog / import dialog respectively
- [ ] 4.4 In `App.kt`, pass `onImportText = viewModel::importItems` when
  rendering the items screen

## 5. Unit tests (JVM)

- [ ] 5.1 Write `ListonicImportParserTest`: strips `•`, `-`, and `*` bullets;
...
- [ ] 5.2 Extend `FakeShoppingItemRepository` with `createAll` (append each name
  with an increasing `createdAt`, re-sort through the existing `order` helper)
- [ ] 5.3 Extend `ItemsViewModelTest`: successful import appends all parsed
...
- [ ] 6.1 Extend `RoomShoppingItemRepositoryTest`: `createAll` persists every
...
- [ ] 6.2 Add the atomicity case: a batch in which at least one item violates
