...
reactive stream filtered by owning list and ordered in two sections — unchecked
items first, then checked items, each by creation time — and support rename,
deletion, bought-flag toggling, and cascade deletion together with their owning
list.
...
### Requirement: Items are observable per list in two-section order
