...
Present the transient combined view of the ShopList app: a single-use screen
that merges items from user-selected lists for shopping, captions each item
with its source list, sorts unchecked-then-bought by name, and writes
check/uncheck changes through to the owning lists.
